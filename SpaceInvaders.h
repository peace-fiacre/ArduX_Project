...
class SpaceInvadersGame : public Game {
  private:
    ...
    struct Bullet { int x, y, px, py; bool active; };
    Bullet bullets[3];
    Bullet enemyBullets[3];

    struct Invader { int x, y, type; bool alive; };
    static const int COLS=6, ROWS=2;
    ...
    void drawInvader(int x, int y, int type, uint16_t color) {
      screen->fillRect(x, y, 13, 10, TFT_BLACK); // effacer zone d'abord
      switch(type) {
        case 0:
          screen->fillRect(x+3,y,   6,3,color);
          screen->fillRect(x,  y+3,13,4,color);
          screen->fillRect(x+2,y+7, 3,2,color);
          screen->fillRect(x+8,y+7, 3,2,color); break;
        case 1:
          screen->fillRect(x+2,y,   9,2,color);
          screen->fillRect(x,  y+2,13,5,color);
          screen->fillRect(x+1,y+7, 3,2,color);
          screen->fillRect(x+9,y+7, 3,2,color); break;
        case 2:
          screen->fillRect(x+1,y,  11,3,color);
          screen->fillRect(x,  y+3,13,4,color);
          screen->fillRect(x,  y+7, 4,2,color);
          screen->fillRect(x+9,y+7, 4,2,color); break;
      }
    }
    ...
    void initWave() {
      for(int r=0;r<ROWS;r++) for(int c=0;c<COLS;c++) {
        invaders[r][c] = { 20 + c*30, 25 + r*20, r, true };
      }
      invaderDirX=1; invaderOffsetX=0; invaderOffsetY=0;
      invaderMoveInterval = max(200, 600 - wave*30);
  ...
  public:
    ...
    void init() override {
      shipX=112; prevShipX=112;
      score=0; state=IN_PROGRESS;
      firstDraw=true; lastUpdate=0;
      lastShot=0; lastEnemyShot=0; wave=1;
      for(int i=0;i<3;i++){
    ...
    void update(Buttons buttons) override {
      ...
      if(buttons.aPressed && millis()-lastShot>300) {
        ...
        for(int i=0;i<3;i++) if(!bullets[i].active) {
          bullets[i]={shipX+shipW/2, shipY-4, shipX+shipW/2, shipY-4, true};
      ...
      for(int i=0;i<3;i++) {
        ...
        for(int r=0;r<ROWS;r++) for(int c=0;c<COLS;c++) {
          ...
          if(bullets[i].x>=ix && bullets[i].x<=ix+12 &&
            ...
            bullets[i].active=false;
            score+=(ROWS-r)*10*wave;
            aliveCount--;
            tone(SI_BUZZER,400,100);
            // Effacer invader mort
            screen->fillRect(ix-1,iy-1,16,13,TFT_BLACK);
            for(int e=0;e<4;e++) if(!explosions[e].active){
              explosions[e]={ix,iy,6,true}; break;
            }
          }
        }
      }
      ...
      if(millis()-lastInvaderMove>invaderMoveInterval) {
        ...
        for(int r=0;r<ROWS;r++) for(int c=0;c<COLS;c++) {
          if(!invaders[r][c].alive) continue;
          int ix=invaders[r][c].x+invaderOffsetX;
          if(ix+13>maxX) maxX=ix+13;
          if(ix<minX)    minX=ix;
        }
        if((invaderDirX==1&&maxX>=238)||(invaderDirX==-1&&minX<=2)){
          invaderDirX=-invaderDirX;
          invaderOffsetY+=8;
          tone(SI_BUZZER,150,80);
        } else {
          invaderOffsetX+=invaderDirX*2;
        }
        ...
        for(int tries=0;tries<20;tries++) {
          ...
          if(invaders[r][c].alive) {
            for(int i=0;i<3;i++) if(!enemyBullets[i].active){
              int ex=invaders[r][c].x+invaderOffsetX+6;
              int ey=invaders[r][c].y+invaderOffsetY+10;
              enemyBullets[i]={ex,ey,ex,ey,true}; break;
            }
            break;
          }
        }
      ...
      for(int i=0;i<3;i++) {
        ...
        enemyBullets[i].px=enemyBullets[i].x;
        enemyBullets[i].py=enemyBullets[i].y;
        enemyBullets[i].y+=3;
      ...
      // Explosions
      for(int e=0;e<4;e++){
        if(explosions[e].active) if(--explosions[e].timer<=0){
          // Effacer explosion
          screen->fillRect(explosions[e].x-2,explosions[e].y-2,20,14,TFT_BLACK);
    ...
    void render() override {
      ...
      if(firstDraw){
        ...
        // étoiles (fond)
        for(int i=0;i<30;i++)
          screen->drawPixel(random(0,240),random(12,112),TFT_WHITE);
      }

      // =======================
      // 🧹 NETTOYAGE ZONE JEU
      // =======================
      // (tout sauf HUD et ligne verte)
      screen->fillRect(0, 12, 240, 120, TFT_BLACK);
